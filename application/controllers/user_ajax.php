...
class user_ajax extends CI_Controller
{
    ...
    {
        $addr_res=$this->db->get_where('tbl_address',array('address_id'=>$address_id));
        $addr_row=$addr_res->result();

        echo json_encode($addr_row);

    }

    public function get_gift_charge($action="")
    {
        if($action=="true")
        {
        ...
        {
            unset($_SESSION["gift_box_rate"]);
        }

        echo "<h1>".$_SESSION["gift_box_rate"]."</h1>";
    ...
    public function get_cod()
    {
        ...
        $_SESSION["cod_charge"]=$conf_rate_row[0]->config_cod_charge; 
        //echo $conf_rate_row[0]->config_cod_charge;
        echo "<td colspan='3'>COD Charges : </td><td><i class='icon-rupee'></i>".$conf_rate_row[0]->config_cod_charge."</td>";
    }

    public function remove_cod()
    {
        unset($_SESSION["cod_charge"]);
    }
    /*
